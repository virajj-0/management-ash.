  ...
  return (
    <main className={`min-h-screen bg-gradient-to-br from-fuchsia-100 via-purple-50 to-pink-100 flex items-center justify-center p-4 transition-all duration-500 ${mounted ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'} ${isExiting ? 'opacity-0 scale-95' : ''}`}>
      <div className="relative bg-white/80 backdrop-blur-sm rounded-3xl p-4 md:p-6 max-w-lg w-full shadow-2xl">
        ...
        <div className="relative w-full aspect-[9/16] bg-gradient-to-br from-fuchsia-200 to-purple-200 rounded-2xl overflow-hidden mb-4">
          {/* Loading indicator */}
          {!isLoaded && (
            <div className="absolute inset-0 flex items-center justify-center z-10">
              <div className="w-12 h-12 border-4 border-fuchsia-300 border-t-fuchsia-600 rounded-full animate-spin"></div>
            </div>
          )}
          ...
          {isLoaded && (
            ...
            >
              <div className={`w-16 h-16 md:w-20 md:h-20 rounded-full bg-white/90 flex items-center justify-center shadow-lg transition-all duration-300 ${isPlaying ? 'opacity-0 scale-75' : 'opacity-100 scale-100 hover:scale-110'}`}>
                {isPlaying ? (
                  <Pause className="w-8 h-8 md:w-10 md:h-10 text-fuchsia-600" />
                ) : (
                  <Play className="w-8 h-8 md:w-10 md:h-10 text-fuchsia-600 ml-1" />
                )}
              </div>
            </button>
          )}
